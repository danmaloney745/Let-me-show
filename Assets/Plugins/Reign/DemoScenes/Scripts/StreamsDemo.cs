...
public class StreamsDemo : MonoBehaviour
{
    ...
    private void saveToPicturesClicked()
    {
        ...
        // File.WriteAllBytes(Application.persistentDataPath + "/" + " Testing.png", data);
        disableButtons();
        uploaded = true;
        StreamManager.LoadFileDialog(FolderLocations.Pictures, 128, 128, new string[] { ".png", ".jpg", ".jpeg" }, imageLoadedCallback);
    ...
    private void imageLoadedCallback(Stream stream, bool succeeded)
    {
        ...
        try
        {
            ...
            if (uploaded == true)
            {
                ...
                {
                    System.IO.Directory.CreateDirectory(Application.persistentDataPath + "/Uploads");
                    uploadfolder = false;
                }
                counter = counter + 1;
                File.WriteAllBytes(Application.persistentDataPath + "/Uploads/" + counter + ".png", data);
                uploaded = false;
            }
            ...
            if (background == true)
            {
                ...
                if (backfolder == true)
                {
                    System.IO.Directory.CreateDirectory(Application.persistentDataPath + "/Backgrounds");
                    backfolder = false;
                }
                counter = counter + 1;
                File.WriteAllBytes(Application.persistentDataPath + "/Backgrounds/" + counter + ".png", data);
                background = false;
            }
        }
        catch (Exception e)
